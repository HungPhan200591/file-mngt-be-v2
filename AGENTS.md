...
1. Session mới, mất context, cần biết trạng thái dự án: dùng `$load-v2-context`.
2. Feature mới hoặc thay đổi nghiệp vụ: dùng `$adlc-feature-delivery`.
3. Đổi REST, Kafka event, database ownership, migration hoặc chạm từ hai service: dùng `$cross-service-contract`.
4. Tạo/sửa rule, context, template, skill hoặc router tài liệu: dùng `$maintain-ai-governance`.
