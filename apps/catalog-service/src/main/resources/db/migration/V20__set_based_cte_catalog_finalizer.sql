-- FT-056 / BT-09D2: durable UNLOGGED scratch keyed by (operation_id, lane_id).
-- Avoids per-page catalog DDL (V19) and repeated stage scans / tmpfs spill (CTE attempt).
-- V19 function body stays immutable.
...
begin
    ...
    join catalog_finalize_page page
      on page.operation_id = target_operation_id and page.lane_id = target_lane_id
     and page.subject_key = stage.subject_key
    ...
    where staged.operation_id = target_operation_id and staged.lane_id = target_lane_id
      and asset.subject_id = staged.subject_id
    ...
    select distinct on (asset.subject_id)
        target_operation_id, target_lane_id, asset.subject_id, asset.id
    from media_asset asset
    join catalog_finalize_page page
      on page.operation_id = target_operation_id and page.lane_id = target_lane_id
     and page.subject_id = asset.subject_id
    left join catalog_finalize_asset staged
      on staged.operation_id = page.operation_id and staged.lane_id = page.lane_id
     and staged.subject_id = asset.subject_id
    ...
    where primary_asset.operation_id = target_operation_id and primary_asset.lane_id = target_lane_id
      and asset.subject_id = primary_asset.subject_id and asset.role = 'PRIMARY_VIDEO'
    ...
    union all
    select target_operation_id, target_lane_id, page.subject_id, stage.payload
    from catalog_finalize_page page
