-- FT-054 follow-up: make the committed V18 finalizer use indexed access paths and
-- materialize canonical state once per subject. This migration is additive so V18
-- remains immutable after it has been applied.

create index if not exists idx_media_subject_identity_key_text
    on media_subject ((region || ':' || subject_type || ':' || identity_key));

create index if not exists idx_catalog_discovery_stage_operation_subject_order
    on catalog_discovery_stage(
...
begin
    ...
    create index idx_catalog_finalize_latest_subject
        on catalog_finalize_latest(subject_key);
    analyze catalog_finalize_latest;
    ...
    create index idx_catalog_finalize_asset_locator
        on catalog_finalize_asset(subject_id, storage_key, relative_path);
    analyze catalog_finalize_asset;
    ...
    create unique index idx_catalog_finalize_primary_subject
        on catalog_finalize_primary(subject_id);
    analyze catalog_finalize_primary;
    ...
    create temporary table catalog_finalize_state on commit drop as
    select page.subject_id, catalog_subject_state_json(page.subject_id) state
    from catalog_finalize_page page;

    create index idx_catalog_finalize_state_subject
        on catalog_finalize_state(subject_id);
    analyze catalog_finalize_state;

    update catalog_finalize_page page
    set after_hash = md5(state.state::text)
    from catalog_finalize_state state
    where state.subject_id = page.subject_id;
    ...
    join catalog_finalize_latest latest using (subject_key)
    join catalog_finalize_state state using (subject_id);
