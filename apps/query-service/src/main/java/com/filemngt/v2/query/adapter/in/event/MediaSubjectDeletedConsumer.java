...
@Component
public class MediaSubjectDeletedConsumer {
    private final ObjectMapper json;
    private final QueryProjectionService service;

    public MediaSubjectDeletedConsumer(ObjectMapper json, QueryProjectionService service) {
        this.json = json;
        this.service = service;
    }

    @KafkaListener(
            topics = "media.subject.deleted.v1",
    ...
    public void consume(ConsumerRecord<String, String> record) throws JacksonException {
        try (var ignored = KafkaTracingHeaderPropagation.extractAndSetMdc(record)) {
            service.handle(json.readValue(record.value(), MediaSubjectDeletedV1.class));
        }
    }
}
