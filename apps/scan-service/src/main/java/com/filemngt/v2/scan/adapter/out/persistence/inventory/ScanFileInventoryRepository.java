...
public interface ScanFileInventoryRepository extends JpaRepository<ScanFileInventoryEntity, UUID> {
    ...
    /**
     * Lấy snapshot nhẹ (path, fingerprint và state) để so sánh với file trên đĩa.
     * Dùng trong reconciliation chunk bounded-memory: 1 câu IN thay vì N+1 query.
     */
    @Query("""
            SELECT new com.filemngt.v2.scan.domain.inventory.ScanInventorySnapshot(
                e.sourceRelativePath, e.fileSize, e.fileModifiedAt, e.state)
