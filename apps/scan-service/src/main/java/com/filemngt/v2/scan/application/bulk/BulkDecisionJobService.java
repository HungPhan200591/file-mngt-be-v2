...
public class BulkDecisionJobService {
    ...
    public ScanAsyncJobStatus status(UUID jobId) {
        var job = jobs.findById(jobId).orElseThrow(() -> new IllegalArgumentException("Unknown bulk decision job: " + jobId));
        return new ScanAsyncJobStatus(job.id(), "BULK_DECISION", job.status(), job.processedCount(), job.createdAt(),
                job.startedAt(), job.finishedAt(), job.lastError(), null);
    }
}
