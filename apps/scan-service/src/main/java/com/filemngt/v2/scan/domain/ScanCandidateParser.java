...
public final class ScanCandidateParser {
    ...
    public static ScanCandidate parse(ScanProfile profile, String relativePath) {
        ...
        ScanCandidateType type = SemanticParserSupport.candidateType(profile);
        ScanAssetRole role =
                switch (type) {
    ...
    /** Ánh xạ profile nội bộ sang vùng registry Catalog cần dùng khi scan. */
    public static ScanRegion region(ScanProfile profile) {
        return switch (profile) {
            case JOKE_VIDEO, JOKE_ASSET -> ScanRegion.JOKE;
            case USE_VIDEO, USE_ASSET, USE_ALBUM -> ScanRegion.USE;
        };
    }
