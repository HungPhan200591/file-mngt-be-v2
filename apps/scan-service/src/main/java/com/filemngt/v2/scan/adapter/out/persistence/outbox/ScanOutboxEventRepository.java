...
public interface ScanOutboxEventRepository extends JpaRepository<ScanOutboxEventEntity, UUID> {
    ...
    @Query(value = "select * from scan_outbox_event where published_at is null and (lease_until is null or lease_until < :now) order by created_at, id limit :limit for update skip locked", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query("update ScanOutboxEventEntity e set e.publishedAt = :now, e.lastError = null, e.leaseOwner = null, e.leaseUntil = null where e.id = :id and e.leaseOwner = :owner and e.publishedAt is null")
    int markPublished(@Param("id") UUID id, @Param("owner") String owner, @Param("now") Instant now);

    @Modifying
    @Transactional
    @Query("update ScanOutboxEventEntity e set e.attemptCount = e.attemptCount + 1, e.lastError = :error, e.leaseOwner = null, e.leaseUntil = null where e.id = :id and e.leaseOwner = :owner and e.publishedAt is null")
