...
public class ScanDecisionJdbcRepository {
    ...
    public List<ProposalRow> findPendingChunk(UUID scanRunId, UUID cursor, int limit) {
        String sql = cursor == null
                ? SELECT_FIELDS + " ORDER BY proposal.id LIMIT ?"
                : SELECT_FIELDS + " AND proposal.id > ? ORDER BY proposal.id LIMIT ?";
        Object[] arguments = cursor == null ? new Object[] {scanRunId, limit} : new Object[] {scanRunId, cursor, limit};
