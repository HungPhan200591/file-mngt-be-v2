...
interface IssueRecheckJobRepository extends JpaRepository<IssueRecheckJobEntity, UUID> {
    @Query(value = "select * from scan_issue_recheck_job where status = 'PENDING' or (status = 'RUNNING' and lease_until < :now) order by created_at, id limit 1 for update skip locked", nativeQuery = true)
