...
- Giữ path public tương thích V1: `/files/<drive>:/<path-encoded>`, nhưng dùng origin V2 riêng `http://localhost:18119`. Nginx V2 map `/files/G:/`, `/files/D:/`, `/files/E:/` sang các mount read-only bằng `alias`.
...
- Backend V2 chạy service Compose `nginx-media`, container `file-mngt-v2-nginx-media`, file `infra/nginx/nginx.conf` và host port `18119` riêng. Không mount, dùng chung config hay dùng chung port/container Nginx V1 `nginx_file_mngt` tại `8888`.
...
1. Thêm Nginx V2 root map/container/port tách biệt và kiểm tra URL read-only/range cho fixture.
2. Bổ sung `mediaUrl` additive vào DTO read model. URL này do V2 tạo từ locator và root map, có origin `http://localhost:18119` và path `/files/<drive>:/...`; chốt REST/event projection contract trong feature migration.
