...
4. **`BT-09D — Catalog batch/coalesce` (`D1 DONE` / `D2 READY`)**: [FT-055](./features/055-catalog-typed-ingest/03-plan.md) ingest `DONE`. Isolated 1M **`20.464 s` (~48.866 rec/s)**; Kafka backlog-drain 1M **`24.527 s` (~40.771 rec/s)**. Lát tiếp theo là [FT-056 / BT-09D2 Set-Based CTE Merge](./features/056-catalog-set-based-cte-merge/03-plan.md) (`READY`) — xóa per-page temp DDL trong `catalog_finalize_operation_page`. Không phải watermark gate. D3 lane drain và D4 relay chưa mở.
...
1. **Triển khai [FT-056 / BT-09D2 — Set-Based CTE Merge](./features/056-catalog-set-based-cte-merge/03-plan.md)** (`READY`): viết lại `catalog_finalize_operation_page` bằng CTE, bỏ temp DDL/index/ANALYZE mỗi page; IT parity rồi isolated merge benchmark 2.500 / 100.000 subject. Gate `pageExec` median `< 5 ms` và merge 100K subject `<= 5 s`.
