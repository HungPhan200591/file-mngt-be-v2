...
4. **`BT-09D — Catalog batch/coalesce` (`FT-054 IMPLEMENTING — verification/qualification pending`)**: [FT-054](./features/054-catalog-operation-coalescing/03-plan.md) đã có shared contracts, Scan stage-10 watermark, durable set-based batch ingest/equality gate, 64 fenced native canonical lane, one-final-snapshot v2, DLT/input watchdog, output pressure gate và native continuous relay. Source/benchmark harness đã hoàn thiện nhưng chưa chạy migration, focused test, real-Kafka relay hoặc 25k/1M qualification. Feature chỉ `DONE` khi canonical 1M `<= 10s`, relay `<= 2s` cùng correctness/failure gate; không tách thêm FT chỉ để hoàn tất throughput BT-09D.
