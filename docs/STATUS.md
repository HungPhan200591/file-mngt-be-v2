...
4. **`BT-09D — Catalog batch/coalesce` (`D1 DONE` / `D2 READY — V22 recovery`)**: [FT-055](./features/055-catalog-typed-ingest/03-plan.md) ingest `DONE`. [FT-056](./features/056-catalog-set-based-cte-merge/03-plan.md) V20/V21 **immutable** và đều fail: V20 25K **`2.633 s`** / avg **129 ms**, 1M gãy connection; V21 user báo chậm hơn V19 và 1M timeout (exact metrics pending). Plan V22 dùng durable typed reduction + direct merge, recovery gate 1M `< 60 s`. D3/D4 chưa mở.
...
1. **Triển khai [FT-056](./features/056-catalog-set-based-cte-merge/03-plan.md) V22 theo P0 → P3**: khóa phase profile V21, thêm durable typed reduction, direct canonical merge rồi chạy 25K → 250K → 1M. Recovery gate: cả ba run 1M `< 60 s`; gate lịch sử `<= 5 s` chưa tuyên bố.
