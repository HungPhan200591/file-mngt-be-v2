...
4. **`BT-09D — Catalog batch/coalesce` (`D1 DONE` / `D2 IMPLEMENTED — qualification pending`)**: [FT-055](./features/055-catalog-typed-ingest/03-plan.md) ingest `DONE`. [FT-056](./features/056-catalog-set-based-cte-merge/03-plan.md) có V22/V22.1 logged typed reduction, rebuild từ raw stage và direct finalizer; migration/Testcontainers/benchmark chưa chạy. V20/V21 **immutable** và đều fail: V20 25K **`2.633 s`** / avg **129 ms**, 1M gãy connection; V21 user báo chậm hơn V19 và 1M timeout (exact metrics pending). Recovery gate V22 là ba run 1M `< 60 s`. D3/D4 chưa mở.
...
1. **Qualification [FT-056](./features/056-catalog-set-based-cte-merge/03-plan.md) V22 theo P0 → P3**: chạy Flyway/Testcontainers rồi benchmark warm-up + ba run 25K → 250K → 1M, kèm phase/resource manifest. Recovery gate: cả ba run 1M `< 60 s`; gate lịch sử `<= 5 s` chưa tuyên bố.
