...
4. **`BT-09D — Catalog batch/coalesce` (`FT-054 QUALIFICATION FAILED — optimization in progress`)**: [FT-054](./features/054-catalog-operation-coalescing/03-plan.md) đã có shared contracts, Scan stage-10 watermark, durable set-based batch ingest/equality gate, 64 fenced native canonical lane, one-final-snapshot v2, DLT/input watchdog, output pressure gate và native continuous relay. User-provided run đạt `25K / 5.781s / 4.325 records/s`; run `1M` chưa hội tụ và các finalizer lane gặp `QueryTimeoutException`. V19 access-path optimization, failed-lane release, last-lane completion guard và phase timing diagnostics đã được thêm nhưng chưa chạy migration/build/test. Feature chỉ `DONE` khi canonical 1M `<= 10s`, relay `<= 2s` cùng correctness/failure gate; không tách thêm FT chỉ để hoàn tất throughput BT-09D.
