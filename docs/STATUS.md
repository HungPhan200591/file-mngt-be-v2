...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-058 IMPLEMENTED — verification pending`)**: FT-057 data plane đã implement và correctness IT 7+3+9 pass, nhưng combined 25K ngày 2026-08-22 phát hiện seal deadlock và DLT partition mismatch, operation kẹt `INGESTING`; run không tạo throughput evidence hợp lệ. [FT-058](./features/058-catalog-operation-reliability-hardening/03-plan.md) đã tách seal coordinator, harden retry/DLT, thêm total deadline 120s và concurrent/failure tests. Chưa chạy build/IT/migration/benchmark sau implementation; không rewrite reconciliation SQL trước khi reliability gate pass.
...
1. **Verify FT-058 Catalog reliability hardening** theo đúng thứ tự: compile/unit → PostgreSQL IT → Kafka failure IT → benchmark 25K rồi ba run 1M.
