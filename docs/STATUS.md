...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-063 DONE — V28 STABLE`)**: V28 local combined 25K đạt `7.765 ms`. Follow-up event-driven đã code và qua correctness/recovery nhưng lượt cuối chỉ đạt `7.391 ms` (nhanh hơn `4,8%`), không đạt ceiling 4 giây hay gate giữ candidate `20%`; candidate đã rollback theo [plan](./features/063-catalog-reconciliation-page-access-paths/04-25k-event-driven-happy-path-plan.md). Không chạy 250K/1M. Throughput vẫn `UNQUALIFIED`; `TD-023` defer tối ưu Catalog.
