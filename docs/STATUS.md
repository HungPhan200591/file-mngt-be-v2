...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-061 correctness pass; physical performance failed`)**: FT-061 thay V59 per-slice parent exclusive lock bằng shared ingest fence, derived progress recount và blocked-child propagation. Targeted regression 35/35; 25K x3 với bốn ingest workers đạt `2.688–3.307 ms`, exact cardinality và zero lock wait/deadlock. Physical 1M vượt `110s` khi PostgreSQL vẫn active ở bulk-upsert synchronization nên đã dừng theo gate; không chạy combined. Xem [evidence](../apps/catalog-service/src/test/java/com/filemngt/v2/catalog/benchmark/results/08-ft061-shared-ingest-fence.md).
...
1. **FT-062 bulk-upsert/synchronization write shape (`PLANNED`)**: FT-061 đã sửa ingest contention nhưng physical 1M vẫn vượt 110 giây tại bulk-upsert synchronization. Giữ stable correctness baseline; nếu tiếp tục BT-09D thì chỉ thay write shape của phase này, không tối ưu ingest vòng hai hoặc tăng worker mù.
