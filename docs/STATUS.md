...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-058 FEASIBILITY_FAILED`)**: Reliability source đã hoàn tất và compile + unit + PostgreSQL/Kafka regression đạt `36/36 PASS`; Flyway V24 migrate thành công trên PostgreSQL 18 Testcontainers. Combined 25K ngày 2026-08-22 hoàn tất trong `4.935 ms` (`5.066 input records/s`) nhưng không đạt 30K stretch. Combined 1M không hoàn tất trong deadline 120 giây: reconciliation units `0–3` lặp lại `QueryTimeoutException` với statement timeout 20 giây và operation còn `RECONCILING`. Current 16-unit set-based shape vì vậy không đạt release gate 1M/120s; theo [FT-058](./features/058-catalog-operation-reliability-hardening/03-plan.md), dừng tối ưu cùng shape và chuyển sang feature mới cho partition/shard completion contract.
...
1. **Đóng FT-058 ở `FEASIBILITY_FAILED` và mở feature kiến trúc kế tiếp cho Catalog partition/shard completion contract**; không tăng timeout, retry, worker count hoặc tạo thêm reconciliation SQL candidate cùng 16-unit transaction shape.
...
3. **Chỉ mở `BT-09E` sau khi Catalog có candidate mới hoàn tất 1M trong 120 giây** với exact cardinality và final broker acknowledgement. Sau đó tiếp tục **`BT-09F`** → **`BT-09G`**; một local run không được diễn giải thành production capacity evidence.
