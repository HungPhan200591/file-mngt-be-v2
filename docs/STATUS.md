...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-060 FEASIBILITY_FAILED; FT-059 correctness stable`)**: FT-059 logical shard contract và stable mode đã khóa marker/data race; combined 25K đạt 3/3. FT-060 giữ ingest tuần tự và thử bounded bulk upsert. Hai workers là best measured: exact 1M cardinality, zero lock wait/deadlock nhưng `145.586 ms`, vẫn fail 120s. Bốn workers scale âm thành `271.389 ms`; parallel production ingest bị gate bác bỏ do per-slice parent `FOR UPDATE`. Không productionize benchmark candidate. Xem [evidence](../apps/catalog-service/src/test/java/com/filemngt/v2/catalog/benchmark/results/07-ft060-bounded-upsert-parallelism.md).
...
1. **Catalog ingest write-shape decision**: giữ FT-059 stable mode; FT-060 đã bác bỏ tăng writer đơn thuần. Feature tiếp theo phải tách immutable input write khỏi per-slice parent progress, dùng bounded progress fan-in và đo riêng outbox; không chạy combined 250K/1M trước khi physical lower-bound dưới 120 giây, ưu tiên 90 giây.
