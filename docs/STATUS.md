...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-059 READY — implementation pending`)**: [FT-058](./features/058-catalog-operation-reliability-hardening/03-plan.md) dừng ở `FEASIBILITY_FAILED` sau khi combined 1M vượt deadline 120 giây tại transaction khoảng 6.250 subjects/unit. [FT-059](./features/059-catalog-logical-shard-completion/03-plan.md) đã chốt architecture/contract additive: logical shard theo canonical subject key, transactional `media.approval.shard.completed.v1`, Catalog shard equality gate và bounded page checkpoint để overlap ingest → reconciliation → relay → Query projection. Candidate 64 shards/page 250–500 chỉ là hypothesis; chưa có source hoặc benchmark qualification.
...
1. **Triển khai [FT-059](./features/059-catalog-logical-shard-completion/03-plan.md) theo contract/ADR đã chốt**: shared routing golden vectors → Scan transactional shard marker → Catalog shard equality + bounded page checkpoint → correctness/reliability → 25K/250K calibration → 1M qualification. Không tăng timeout, retry, worker count hoặc tạo thêm SQL candidate cùng 16-unit transaction shape.
...
3. **Chỉ mở `BT-09E` sau khi FT-059 hoàn tất ba run 1M trong 120 giây** với exact cardinality, zero unresolved DLT, resource bounded và final broker acknowledgement. Sau đó tiếp tục **`BT-09F`** → **`BT-09G`**; local qualification không được diễn giải thành production capacity evidence.
