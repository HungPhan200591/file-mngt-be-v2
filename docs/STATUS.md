...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-059 IMPLEMENTED — stable 25K verified; scale qualification pending`)**: [FT-058](./features/058-catalog-operation-reliability-hardening/03-plan.md) dừng ở `FEASIBILITY_FAILED`. [FT-059](./features/059-catalog-logical-shard-completion/03-plan.md) đã triển khai logical shard contract và stable mode tuần tự; race marker/data dưới PostgreSQL `READ COMMITTED` đã được khóa bằng IT. Combined 25K tới final broker acknowledgement đạt 3/3 lượt độc lập trong `25.492–31.407 ms` (796–981 records/s). Đây là correctness/stability evidence cục bộ, chưa đạt throughput target hoặc qualification 250K/1M.
...
1. **Qualification [FT-059](./features/059-catalog-logical-shard-completion/03-plan.md)**: giữ stable mode làm correctness baseline; tiếp tục reliability/crash-reclaim rồi 250K calibration → ba run 1M. Chỉ tăng concurrency sau khi có test race tương ứng; giữ exact cardinality, zero unresolved DLT và resource bounded.
