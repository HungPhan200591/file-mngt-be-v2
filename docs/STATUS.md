...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-059 IMPLEMENTED — correctness stable; serial 1M feasibility failed`)**: FT-059 logical shard contract và stable mode đã khóa marker/data race; combined 25K đạt 3/3. Sequential physical lower-bound 1M (không scheduler/Kafka/overlap) đạt exact `1M input / 100K subject / 1M asset / 100K outbox` nhưng mất `171.871 ms`, trong đó ingest `68.472 ms` và bulk upsert `62.902 ms`. Zero lock wait/deadlock, heap/GC thấp và CPU directional sample thấp: serial shape không đạt 120 giây, chưa kết luận local hardware hết capacity. Xem [evidence](../apps/catalog-service/src/test/java/com/filemngt/v2/catalog/benchmark/results/06-ft059-sequential-physical-feasibility.md).
...
1. **FT-059 data-plane decision**: giữ stable mode làm correctness baseline; không chạy combined 250K/1M trên serial shape. Candidate tiếp theo chỉ thử bounded intra-phase parallelism `2 → 4` cho ingest/upsert, không overlap DB-heavy stage; gate 25K correctness 3/3 rồi chạy lại sequential physical 1M. Dừng nếu lower-bound vẫn vượt 120 giây hoặc có lock wait/deadlock/resource unbounded.
