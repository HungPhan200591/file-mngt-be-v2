...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-059 IMPLEMENTED — targeted verification passed; qualification pending`)**: [FT-058](./features/058-catalog-operation-reliability-hardening/03-plan.md) dừng ở `FEASIBILITY_FAILED` sau khi combined 1M vượt deadline 120 giây tại transaction khoảng 6.250 subjects/unit. [FT-059](./features/059-catalog-logical-shard-completion/03-plan.md) đã triển khai routing canonical subject key, Scan transactional `media.approval.shard.completed.v1`, Catalog shard equality gate, per-shard DLT gate và bounded page checkpoint để overlap ingest → reconciliation → relay → Query projection. Candidate 64 shards/page 250–500 vẫn là hypothesis; chưa có benchmark hoặc scale qualification.
...
1. **Qualification [FT-059](./features/059-catalog-logical-shard-completion/03-plan.md)**: tiếp tục reliability/crash-reclaim rồi 25K/250K calibration → ba run 1M; giữ exact cardinality, zero unresolved DLT, resource bounded và không tăng timeout, retry, worker count hoặc quay về SQL candidate 16-unit.
