...
4. **`BT-09D — Catalog batch/coalesce` (`D1 DONE` / `D2 READY`)**: [FT-055](./features/055-catalog-typed-ingest/03-plan.md) ingest `DONE`. Isolated 1M **`20.464 s` (~48.866 rec/s)**; Kafka backlog-drain 1M **`24.527 s` (~40.771 rec/s)**. [FT-056](./features/056-catalog-set-based-cte-merge/03-plan.md) V19 merge baseline: 2.500 subject **`2.032 s`** (pageExec avg **106 ms**); 100.000 subject **timeout > 2 min**. Chưa viết V20 CTE. D3/D4 chưa mở.
