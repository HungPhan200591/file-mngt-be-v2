...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-063 DONE; throughput UNQUALIFIED`)**: [FT-063](./features/063-catalog-reconciliation-page-access-paths/03-plan.md) thêm hai page-aligned winner indexes mà không đổi reducer/contract. Targeted IT đạt 12/12; một local combined 25K hoàn tất tới final broker ACK trong `7.765 ms` (`3.220 records/s`), giảm 29,3% so với baseline `10.981 ms`. Đây không phải repeated-run, 1M hoặc production qualification; `TD-023` vẫn mở.
