...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-063 25K FAST PATH READY_TO_IMPLEMENT`)**: V28 indexes đã đưa local combined 25K xuống `7.765 ms`, nhưng khoảng `3.548 ms` còn nằm ngoài data processing chính. [Kế hoạch duy nhất](./features/063-catalog-reconciliation-page-access-paths/04-25k-event-driven-happy-path-plan.md) ưu tiên event-driven operation progress: direct signal sau commit/ACK, scheduler chỉ recovery fallback, target <=3 giây và acceptance ceiling <=4 giây. Tối đa hai benchmark 25K mới; không chạy 250K/1M. Throughput vẫn `UNQUALIFIED`; `TD-023` còn mở.
...
1. **FT-063 25K fast path (`READY_TO_IMPLEMENT`)**: thực hiện [plan P1 → P7](./features/063-catalog-reconciliation-page-access-paths/04-25k-event-driven-happy-path-plan.md) một mạch: operation-scoped progress → direct post-commit/ACK signals → relay wake → correctness/recovery → benchmark 25K → tối đa một residual fix → đóng feature.
2. **BT-09E Query bulk projection (`READY TO OPEN`)**: bắt đầu ngay sau khi FT-063 follow-up đóng. Tiếp tục batch consumer, staging/COPY hoặc set-based upsert, version guard và processed-event watermark; throughput vẫn `UNQUALIFIED`.
3. **Qualification còn mở của BT-09C:** [FT-053](./features/053-lane-fenced-outbox-data-plane/03-plan.md) đã vượt isolated immediate-ack floor nhưng vẫn cần real-Kafka, representative payload, repeated-run và crash/reclaim/broker-failure evidence; giữ `TD-013` active.
4. Sau BT-09E tiếp tục **BT-09F** correctness/failure evidence rồi **BT-09G** scale qualification. Exact cardinality,
   zero unresolved DLT và final broker acknowledgement vẫn bắt buộc; elapsed local không còn block functional flow.
5. **Giai đoạn sau khi thông luồng SC-01:** thực hiện Hardening P0 (`TD-009` → `TD-012`), chạy Testcontainers / Flyway / DLT verification, chốt E2E Gateway/FE cutover.
6. **Giai đoạn phát triển tính năng mới:** triển khai **Phase 4 Media Worker** ([FT-013](./features/013-media-worker-processing-foundation/03-plan.md)) → **Phase 7 Importer V1** → **Phase 8 Observability mở rộng**.
