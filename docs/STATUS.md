...
4. **`BT-09D — Catalog bulk reconciliation` (`FT-058 FUNCTIONAL VERIFIED — benchmark pending`)**: FT-057 data plane đã implement, nhưng combined 25K ngày 2026-08-22 từng phát hiện seal/DLT failure rồi lock-upgrade deadlock giữa các reconciliation units nên các run cũ không tạo throughput evidence hợp lệ. [FT-058](./features/058-catalog-operation-reliability-hardening/03-plan.md) đã tách seal coordinator, harden retry/DLT, bỏ shared operation lock trước concurrent checkpoint, thêm total deadline 120s và concurrent/failure tests. Compile + unit + PostgreSQL/Kafka regression đạt `36/36 PASS`; Flyway V24 migrate thành công trên PostgreSQL 18 Testcontainers. Benchmark 25K/1M cần chạy lại nên chưa có throughput evidence; không rewrite reconciliation SQL trước khi reliability gate pass.
...
1. **Benchmark FT-058 Catalog reliability hardening** theo đúng thứ tự: combined 25K → combined 1M; cả hai dùng deadline operation 120 giây và không tính seed/warm-up.
...
3. **Sau FT-058:** nếu run 1M `<= 120s`, ghi nhận local qualification rồi đóng băng Catalog và mở **`BT-09E`** (Query bulk) → **`BT-09F`** → **`BT-09G`**. Nếu vượt 120s, mở contract partition/shard completion; không tạo thêm SQL candidate cùng shape. Một local run không được diễn giải thành production capacity evidence.
