...
| `TD-023` | `catalog-service`, `platform/data` | MEDIUM | Catalog throughput còn `UNQUALIFIED`. FT-064 hybrid 25K đạt `7.696 ms`, performance-neutral so với V28 `7.765 ms`; Java reduce chỉ `34 ms`, COPY `149 ms`, còn set-based canonical/relationship/snapshot apply `2.203 ms` và coordination/relay. | Defer sau BT-09E/F. Lần mở tiếp theo phải tách canonical mutation khỏi snapshot JSON/outbox bằng evidence statement-level, có workload/resource/SLO; không tăng DB writer concurrency hoặc chạy 250K/1M từ FT-064. |
