...
| `TD-013` | `scan-service`, `catalog-service` | HIGH | Outbox publish tuần tự; Scan dùng `join()` không timeout, claim batch có thể vượt lease 30 giây. | Bounded in-flight/async ack, timeout/backpressure, lease budget và throughput/duplicate benchmark. |
