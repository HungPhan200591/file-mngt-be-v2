...
| `TD-023` | `catalog-service`, `platform/data` | MEDIUM | Catalog throughput còn `UNQUALIFIED`. FT-064 hybrid 25K đạt `7.696 ms`, performance-neutral so với V28 `7.765 ms`. Lượt 1M hoàn tất exact + final ACK trong `224.954 ms`, fail target 120 giây; 40 unit có execution sum `123.205 ms`, trong đó SQL apply `111.313 ms` (~`90,3%`). | Defer sau BT-09E/F. Lần mở tiếp theo phải tách canonical mutation khỏi snapshot JSON/outbox bằng statement-level evidence và workload/resource/SLO rõ ràng; không tăng DB writer concurrency theo suy đoán. |
