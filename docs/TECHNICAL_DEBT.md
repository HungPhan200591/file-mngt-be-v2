...
| `TD-023` | `catalog-service`, `platform/data` | MEDIUM | Catalog throughput còn `UNQUALIFIED`; FT-063 V28 local 25K đạt `7.765 ms`. Event-driven candidate đã qua correctness nhưng chỉ đạt `7.391 ms` (`4,8%` gain), không qua gate và đã rollback; coordination + 64-lane relay vẫn chi phối phần latency còn lại. | Defer tối ưu Catalog sau BT-09E/F. Chỉ mở feature mới khi chốt workload, resource budget, SLO/cost ceiling và measurement tách seed/ingest/reconcile/relay; không tiếp tục tuning FT-063 hoặc chạy 250K/1M từ plan đã đóng. |
