...
| ↳ **BT-09D1 — Fast Ingest Path** | Typed CSV COPY, giữ raw payload, precompute stable lane hash trong Java và bỏ wrapper JSON parse/cast trong stage SQL. | **`READY`** ([FT-055](../../../../../docs/features/055-catalog-typed-ingest/03-plan.md)) | 25K `stageSql` median `<= 100ms`, max `<= 150ms`; ingest 1M `<= 4s` (`>= 250k rec/s`). |
