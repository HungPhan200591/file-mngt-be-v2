...
| **BT-09D — Catalog batch/coalesce** | Durable operation staging, equality gate, native canonical merge và one-final-snapshot relay. FT-054 monolithic **`CLOSED — QUALIFICATION FAILED`**; tối ưu tiếp theo bốn gate tuần tự. | **`D1 READY`** | D1 → D4 phải pass riêng; Catalog canonical `<= 10s`, relay `<= 2s` theo SLO budget owner. |
...
| ↳ **BT-09D2 — Set-Based CTE Merge** | Xóa per-page DDL storm (7 temp tables, 4 index, 4 analyze), dùng in-query set-based CTE và bỏ `before_hash` cho subject mới. | `PLANNED` | SQL merge median `< 5ms/page`; 100K subjects `<= 5s`, correctness parity và không temp-DDL trong page loop. |
