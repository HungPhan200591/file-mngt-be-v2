...
| **BT-09D — Catalog batch/coalesce** | Durable operation staging, equality gate, native canonical merge và one-final-snapshot relay. (FT-054 monolithic **`CLOSED — QUALIFICATION FAILED`**). Phân tách thành 4 sub-tasks độc lập: | **`SUB-TASKS PLANNED`** | Từng sub-task có feature và benchmark gate riêng; không làm monolithic one-shot. |
| ↳ **BT-09D1 — Fast Ingest Path** | Typed Streaming TSV/CSV COPY, precompute hash lane trong Java, loại bỏ 13 phép parse JSONB trong SQL. | `PLANNED` | Giảm `stageSql` từ 1.211ms xuống < 150ms (25k); Ingest 1M <= 3-4s (>= 250k rec/s). |
| ↳ **BT-09D2 — Finalizer CTE Merge** | Xóa bỏ DDL storm (7 temp tables, 4 index, 4 analyze), chuyển sang in-memory CTE, bypass hash cho subject mới. | `PLANNED` | Giảm SP latency từ 144ms/page xuống < 5ms/page; merge 100k subjects <= 4-5s. |
| ↳ **BT-09D3 — Lane Drain Concurrency** | Drain liên tục toàn bộ page trong 1 worker claim, loại bỏ 2.054ms acquire lock contention, tinh chỉnh worker/page size. | `PLANNED` | Zero acquire contention; hoàn tất drain 64 lanes ổn định không deadlock. |
| ↳ **BT-09D4 — Relay & 1M Qualification** | Native continuous relay 64 virtual lanes, batch broker ack & mark, benchmark qualification 1M hoàn chỉnh. | `PLANNED` | Đạt throughput 30k-40k rec/s (1M <= 25-30s), zero data loss/DLT, hoàn tất `CATALOG_COMMITTED`. |
