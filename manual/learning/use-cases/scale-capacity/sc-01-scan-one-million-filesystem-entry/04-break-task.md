...
| **BT-09D — Catalog batch/coalesce** | Durable operation staging, equality gate, native canonical merge và one-final-snapshot relay. FT-054 monolithic **`CLOSED — QUALIFICATION FAILED`**; tối ưu tiếp theo bốn gate tuần tự. | **`D1 DONE` / `D2 READY`** | D1 → D4 phải pass riêng; Catalog canonical `<= 10s`, relay `<= 2s` theo SLO budget owner. |
| ↳ **BT-09D1 — Fast Ingest Path** | Typed CSV COPY, giữ raw payload, precompute stable lane hash trong Java và bỏ wrapper JSON parse/cast trong stage SQL. | **`DONE`** ([FT-055](../../../../../docs/features/055-catalog-typed-ingest/03-plan.md)) | Isolated Ingest 1M đạt `20.4s` (~48.8k rec/s); Kafka backlog-drain 1M đạt `24.5s` (~40.8k rec/s) với 8 partition / 8 consumer, slice 5000. |
| ↳ **BT-09D2 — Set-Based CTE Merge** | Xóa per-page DDL storm (7 temp tables, 4 index, 5 analyze), dùng in-query set-based CTE và bỏ `before_hash` cho subject mới. | **`READY`** ([FT-056](../../../../../docs/features/056-catalog-set-based-cte-merge/03-plan.md)) | SQL merge median `< 5ms/page`; 100K subjects `<= 5s`, correctness parity và không temp-DDL trong page loop. |
| ↳ **BT-09D3 — Continuous Lane Drain** | Một claim drain nhiều page đến empty/deadline, không release/reacquire sau mỗi page; tune worker/page theo pool và lease budget. | `PLANNED` | Tổng acquire/lease overhead `< 5%` finalizer elapsed; 64 lane hoàn tất không deadlock/starvation/fence violation. |
| ↳ **BT-09D4 — Relay & Catalog 1M Gate** | Native continuous relay 64 virtual lanes, bounded broker ack + fenced bulk mark và qualification Catalog hoàn chỉnh. | `PLANNED` | Canonical `<= 10s`, relay `<= 2s`, Catalog phase `<= 12s`; data loss/duplicate canonical effect/unresolved DLT đều `0`, `CATALOG_COMMITTED` được broker-ack + durable-mark. |
