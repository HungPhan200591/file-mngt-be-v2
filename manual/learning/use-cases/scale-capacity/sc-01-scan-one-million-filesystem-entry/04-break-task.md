...
| **BT-09D — Catalog bulk reconciliation** | Giữ durable stage/equality/outbox semantics nhưng thay incremental reduction và page-loop bằng append-only ingest, one-time reduction, coarse canonical merge và indexed sliding relay. FT-054–FT-056 là failed/historical evidence. | **`READY`** ([FT-057](../../../../../docs/features/057-catalog-bulk-reconciliation-data-plane/03-plan.md)) | Một combined clock từ first Catalog receive tới final broker ack: 1M `<= 33.334 ms` (`>= 30K/s`), stretch `<= 25.000 ms` (`>= 40K/s`); exact correctness và resource bounded. |
