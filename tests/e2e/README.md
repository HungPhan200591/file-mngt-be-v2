...
Copy `apps/scan-service/src/main/resources/application-local.example.yml` thành `application-local.yml` và sửa path nếu workspace của bạn khác. Khởi động Kafka Compose, `scan-service` và `catalog-service`; Spring Boot tự nạp file local khi chạy application nên không cần EnvFile hoặc environment variable trong IntelliJ. Sau đó tại `tests/e2e/` chạy `npm run scan:local`; scenario CLI tự poll đến `COMPLETED`, kiểm tra proposals/issues, approve idempotent, Catalog tạo đúng một subject/asset và reject không có event. Sau approve, runner chờ Kafka tối đa 10 giây (40 lượt, mỗi lượt 250 ms) và dừng sớm ngay khi Catalog đã có subject. Runner mặc định chỉ in exchange khi fail; dùng `npm run scan:local:debug` khi cần xem từng request/poll.
